 ...
 * DOLLARS.
 */
#define TS_MODULE "[td4100_rmi_dev]"
...
static struct device_attribute attrs[] = {
	__ATTR(open, S_IWUSR | S_IWGRP/*S_IWUSR*/,
			NULL,
			rmidev_sysfs_open_store),
	__ATTR(release, S_IWUSR | S_IWGRP/*S_IWUSR*/,
			NULL,
			rmidev_sysfs_release_store),
	__ATTR(attn_state, S_IRUGO,
			rmidev_sysfs_attn_state_show,
			NULL),
	__ATTR(pid, S_IRUGO | S_IWUSR | S_IWGRP/*S_IWUSR*/,
			rmidev_sysfs_pid_show,
			rmidev_sysfs_pid_store),
	__ATTR(term, S_IWUSR | S_IWGRP/*S_IWUSR*/,
			NULL,
			rmidev_sysfs_term_store),
	__ATTR(intr_mask, S_IRUGO | S_IWUSR | S_IWGRP/*S_IWUSR*/,
			rmidev_sysfs_intr_mask_show,
			rmidev_sysfs_intr_mask_store),
	__ATTR(concurrent, S_IRUGO | S_IWUSR | S_IWGRP/*S_IWUSR*/,
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (ret < 0) {
		TOUCH_E("Failed to read from Page_Select register\n");
		return ret;
	}
	...
	if (page_old == page_new) {
		...
		if (ret < 0) {
			TOUCH_E("Failed to change Page_Select register\n");
			return ret;
		}
	...
	if (ret < 0) {
		TOUCH_E("Failed to write register(addr=0x%04x)\n", addr);
		return ret;
	}
	...
	if (page_changed) {
		...
		if (ret < 0) {
			TOUCH_E("Failed to restore Page_Select register\n");
			return ret;
		}
	}
...
/*	unsigned char intr_status = 0;	*/
	...
	if (length) {
		retval = rmidev_i2c_read(rmidev->ts_dev,
				address,
				(unsigned char *)buf,
				length);
		if (retval < 0) {
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev_i2c_write(rmidev->ts_dev,
				address,
				(unsigned char *)buf,
				length);
		if (retval < 0) {
