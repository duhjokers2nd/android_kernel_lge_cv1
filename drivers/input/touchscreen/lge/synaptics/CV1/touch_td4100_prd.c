...
static int TD4100_Get_P2P_Noise_Data(struct device *dev)
{
	...
	for (i=0; i<ROW_SIZE; i++) {
		for (j=0; j<COL_SIZE; j++) {
			...
			/*
			 * CV1 4th 5.0" BOE Panel characteristics.
			 * Top of row line increase of noise.
			 * Sol : Divide the line result value by 5.
			 */
			if ((boot_mode == MINIOS_MFTS_FOLDER)
				&& (ts->role.mfts_lpwg == 0)
				&& ((i > ROW_SIZE-2) && (j < COL_SIZE))) {
					result_data[i][j] = (result_data[i][j] / 5);
					TOUCH_I("[%s] result_data[%d][%d] : %d\n",
						__func__, i, j, result_data[i][j]);
			}
