...
struct ini_file_data *ini_data = NULL;
...
int ili9881h_get_ini_u8_data(char *key, u8 *buf)
{
	char *s = key;
	char *pToken;
	int ret, conut = 0;
...
static int ili9881h_get_ini_phy_data(const u8 *data, int size)
{
	...
	while (true) {
		...
		//CHECK
		strcpy(ini_data[g_ini_items].pSectionName, tmpSectionName);
...
static void ili9881h_mp_compare_raw_data(int32_t *tmp, int32_t *max_ts, int32_t *min_ts, int* result)
{
	int x, y;
...
static void ili9881h_mp_compare_raw_data_ret(int32_t *tmp, char *csv, int *csv_len,
...
static int ili9881h_mp_cdc_get_pv5_4_command(u8 *cmd, int len, int index)
{
	...
	strncpy(tmp, str, ret);
	ili9881h_get_ini_u8_data(tmp, cmd);
...
static int ili9881h_mp_open_sp_test(int index)
{
	...
	TOUCH_TRACE();
	TOUCH_D(PRODUCTION,"index = %d, name = %s, CMD = 0x%x, Frame Count = %d\n",
...
static int ili9881h_mp_retry_comp_cdc_result(int index)
{
	...
	if(tItems[index].trimmed_mean && tItems[index].catalog != PEAK_TO_PEAK_TEST) {
		...
		ili9881h_mp_compare_raw_data(tItems[index].result_buf, max_threshold, min_threshold, &test_result);
	} else {
		ili9881h_mp_compare_raw_data(tItems[index].max_buf, max_threshold, min_threshold, &test_result);
		ili9881h_mp_compare_raw_data(tItems[index].min_buf, max_threshold, min_threshold, &test_result);
	}
...
void ili9881h_core_mp_show_result(struct device *dev)
{
	...
	for (i = 0; i < core_mp->mp_items; i++) {
		...
		if ( tItems[i].spec_option == BENCHMARK) {
			...
			ili9881h_mp_compare_raw_data_ret(tItems[i].bench_mark_max, csv, &csv_len, TYPE_BENCHMARK, max_threshold, min_threshold,"Max_Bench");
			ili9881h_mp_compare_raw_data_ret(tItems[i].bench_mark_min, csv, &csv_len, TYPE_BENCHMARK, max_threshold, min_threshold,"Min_Bench");
		} else {
			for(j = 0 ;j < core_mp->frame_len ; j++) {
		...
		if (strncmp(tItems[i].name, "open_integration_sp", strlen(tItems[i].name)) == 0) {
			ili9881h_mp_compare_raw_data_ret(frame1_cbk700, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk700");
			ili9881h_mp_compare_raw_data_ret(frame1_cbk250, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk250");
			ili9881h_mp_compare_raw_data_ret(frame1_cbk200, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk200");
		}
		...
		/* Show test result as below */
		if(tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST) {
			ili9881h_mp_compare_raw_data_ret(tItems[i].result_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold,"Mean result");
		} else {
			ili9881h_mp_compare_raw_data_ret(tItems[i].max_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold,"Max Hold");
			ili9881h_mp_compare_raw_data_ret(tItems[i].min_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold,"Min Hold");
		}
		...
		for(j = 0; j < get_frame_count; j++) {
			...
			ili9881h_mp_compare_raw_data_ret(&tItems[i].buf[(j*core_mp->frame_len)], csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, NULL);
		}
	}
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	jitter_ret = ili9881h_core_mp_run_test(dev, "Noise Peak To Peak(With Panel)");
	...
	rawdata_ret = ili9881h_core_mp_run_test(dev, "Raw Data(Have BK)");
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	/* Run Items of MP test */
	lpwg_rawdata_ret = ili9881h_core_mp_run_test(dev, "Raw Data(Have BK) (LCM OFF)");
	...
	if (lpwg_rawdata_ret == 1 && lpwg_jitter_ret == 1) {
		...
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"LPWG RawData : Fail (lpwg_raw:%d/lpwg_jitter:%d)\n",
				(lpwg_rawdata_ret == MP_FAIL)? 0 : 1, (lpwg_jitter_ret == MP_FAIL)? 0 : 1);
		TOUCH_I("LPWG RawData : Fail (lpwg_raw:%d/lpwg_jitter:%d)\n",
				(lpwg_rawdata_ret == MP_FAIL)? 0 : 1, (lpwg_jitter_ret == MP_FAIL)? 0 : 1);
	}
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "==============================\n");
	TOUCH_I("==============================\n");
