...
int ili9881h_sleep_ctrl(struct device *dev, bool start)
{
	...
	data[1] = start ? 0 : 1; // 0x0 : sleep in, 0x1 : sleep out
...
int ili9881h_gesture_ctrl(struct device *dev, int tci_mode, int swipe_mode)
{
	...
	TOUCH_I("%s - gesture ctrl - TCI : %s, SWIPE : %s\n", __func__,
...
int ili9881h_gesture_failreason_ctrl(struct device *dev, int onoff)
{
	...
	TOUCH_I("%s - Lpwg/Swipe Failreason %s\n", __func__, onoff ? "Enable" : "Disable");
...
int ili9881h_check_cdc_busy(struct device *dev, int conut, int delay)
{
...
static int ili9881h_deep_sleep_ctrl(struct device *dev, int state)
{
	...
	if (!atomic_read(&ts->state.incoming_call)) { /* IDLE status */
		if (state == IC_NORMAL) {
			...
			ili9881h_power(dev, POWER_SLEEP);
		}
...
u8 ili9881h_calc_data_checksum(u8 *pMsg, u32 nLength)
{
	int i;
	u32 nCheckSum = 0;
...
static int ili9881h_irq_abs(struct device *dev)
{
	...
	/*//Compare checksum
	check_sum = ili9881h_calc_data_checksum(&data[0], (sizeof(data) - 1));
	if (check_sum != data[DEMO_MODE_PACKET_LENGTH - 1]) {
		TOUCH_E("Packet check sum Error, check_sum = %d, data = %d\n",
				check_sum, data[DEMO_MODE_PACKET_LENGTH - 1]);
	}
	*/
...
static ssize_t store_reg_ctrl(struct device *dev, const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		data = value;
		if (ili9881h_reg_write(dev, reg_addr, &data, sizeof(u32)) < 0)
			TOUCH_E("reg addr 0x%x write fail\n", reg_addr);
		else
			TOUCH_I("reg[%x] = 0x%x\n", reg_addr, data);
	} else if (!strcmp(command, "read")) {
		if (ili9881h_reg_read(dev, reg_addr, &data, sizeof(u32)) < 0)
