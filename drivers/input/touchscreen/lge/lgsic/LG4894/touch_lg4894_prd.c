...
u32 prd_debug_mask = PRD_DEBUG_NONE;
/* PRD_Debug mask value
 * usage: echo [debug_mask] > /sys/module/touch_lg4894_prd/parameters/prd_debug_mask
 */
module_param_named(prd_debug_mask, prd_debug_mask, int, S_IRUGO|S_IWUSR|S_IWGRP);

/*
 * Default : 50000
 * Panel duality model need to devide spec values
 * 50000 -> 100000
 */
static char line[100000];
static char W_Buf[BUF_SIZE];
static u16 M2_Rawdata_buf[M1_M2_RAWDATA_TEST_CNT][tc_total_row * tc_total_col];
static u16 M1_Rawdata_buf[tc_total_row * M1_COL_SIZE];
static u16 open_buf[tc_total_row * tc_total_col];
static u16 short_buf[tc_total_row * SHORT_COL_SIZE];
static u16 LowerImage[tc_total_row][tc_total_col];
static u16 UpperImage[tc_total_row][tc_total_col];
static u16 AverageImage[tc_total_row][tc_total_col];

static int ROW_SIZE = 0;
static int COL_SIZE = 0;
static int COL_ADD_SIZE = 0;

int AIT_RAW_DATA_OFFSET = 0;
int DELTA_DATA_OFFSET = 0;
int LABLE_DATA_OFFSET = 0;
int AIT_BASE_DATA_OFFSET = 0;
int AIT_DEBUG_BUF_DATA_OFFSET = 0;
int PANEL_VENDOR = 0;
...
struct siw_hal_prd_data {
	...
	int prd_app_mode;
	int16_t	m2_buf_rawdata[tc_total_row * tc_total_col];
	int16_t	buf_delta[(tc_total_row + 2) * (tc_total_col + 2)];
	int16_t	buf_debug[tc_total_row * tc_total_col];
	u8	buf_label_tmp[(tc_total_row + 2) * (tc_total_col + 2)];
	u8	buf_label[tc_total_row * tc_total_col];
};
...
#define PRD_SYS_ATTR_EN_FLAG 		(0 |	\
							...
							0)
/* for siw app end */
static const struct prd_param prd_params[] = {
	/*
	 * LG4894 CV group
	 */
	/* Dummy for index 0 */
...
static int prd_param_index = 0;

void prd_info_set(struct device *dev)
{
	struct prd_param *param = (struct prd_param *)prd_params;
	...
	//TOUCH_I("Param Index : %d, F/W Version : v%d.%02d, Pre-Product id : %s / Product id : %s\n",
	//	prd_param_index, d->fw.version[0], d->fw.version[1], d->pre_product_id, d->fw.product_id);

	if(prd_param_index) {
		if (!strncmp(d->pre_product_id, d->fw.product_id, sizeof(d->fw.product_id))) {
			TOUCH_I("Panel is the same. maintain the prd info\n");
			TOUCH_I("prd param: row:%d, col:%d, col_add:%d\n",
					ROW_SIZE, COL_SIZE, COL_ADD_SIZE);
			TOUCH_I("prd offset: rawdata:%X, base:%X, delta:%X, label:%X, debug:%X\n",
					AIT_RAW_DATA_OFFSET, AIT_BASE_DATA_OFFSET, DELTA_DATA_OFFSET,
					LABLE_DATA_OFFSET, AIT_DEBUG_BUF_DATA_OFFSET);
			TOUCH_I("Panel Vendor ID : %d\n", PANEL_VENDOR);
			return;
		}
		TOUCH_I("Panel was changed. get prd info\n");
	}

	/* Maintain previous value.  */
	memcpy(&d->pre_product_id[0], &d->fw.product_id[0], sizeof(d->fw.product_id));

	product_id = d->fw.product_id;
	if (!d->fw.product_id[0]) {
		TOUCH_E("invalid product id, prd_info_set canceled\n");
		return;
	}
	...
	while (1) {
		...
		if (param->product_id == NULL) {
			TOUCH_E("Fail to find matching product id \n");
			return;
		}

		name = (char *)param->product_id;
		len = strlen(name);
		found = !strncmp(product_id, name, len);
		//TOUCH_I("strncmp[%s][%s],%d\n", product_id, name, len);

		if (found) {
			prd_param_index = name_idx;

			ROW_SIZE = param->row;
			COL_SIZE = param->col;
			COL_ADD_SIZE = param->col_add;
			AIT_RAW_DATA_OFFSET = param->rawdata_offset;
			AIT_BASE_DATA_OFFSET = param->base_offset;
			DELTA_DATA_OFFSET = param->delta_offset;
			LABLE_DATA_OFFSET = param->label_offset;
			AIT_DEBUG_BUF_DATA_OFFSET = param->debug_offset;
			PANEL_VENDOR = param->vendor_id;

			TOUCH_I("prd param found: id:%s, row:%d, col:%d, col_add:%d\n",
				product_id, ROW_SIZE, COL_SIZE, COL_ADD_SIZE);
			TOUCH_I("prd offset: rawdata:%X, base:%X, delta:%X, label:%X, debug:%X\n",
				AIT_RAW_DATA_OFFSET, AIT_BASE_DATA_OFFSET, DELTA_DATA_OFFSET,
				LABLE_DATA_OFFSET, AIT_DEBUG_BUF_DATA_OFFSET);
			TOUCH_I("Panel Vendor ID : %d\n", PANEL_VENDOR);
...
*/
	...
	case OPEN_NODE_TEST:
		...
		lg4894_reg_read(dev, tc_tsp_data_access_addr,
							(u8 *)open_buf, ROW_SIZE*(COL_SIZE+COL_ADD_SIZE) * sizeof(u16));
		break;
	case SHORT_NODE_TEST:
		...
		lg4894_reg_read(dev, tc_tsp_data_access_addr,
							(u16 *)short_buf, tc_total_row * SHORT_COL_SIZE * sizeof(u16));
		break;
	}
...
static int prd_print_os_data(struct device *dev, char *buf, u8 type)
{
	...
	switch (type) {
	case OPEN_NODE_TEST:
		col_size = COL_SIZE + COL_ADD_SIZE;
		os_buf = open_buf;
		break;
	case SHORT_NODE_TEST:
	...
	for (i = 0; i < col_size - COL_ADD_SIZE; i++)
...
static int spec_file_read(struct device *dev)
{
	...
	const char *path[2] = {ts->dual_panel_spec[0],
		ts->dual_panel_spec_mfts[0]
	};
#else
	...
	if (ts->dual_panel_spec[0] == NULL || ts->dual_panel_spec_mfts[0] == NULL) {
		TOUCH_I("dual_panel_spec_file name is null\n");
		ret = -1;
		goto error;
	}
#else
...
static int sic_get_limit(struct device *dev,
	char *breakpoint, u16 (*buf)[tc_total_col])
{
	int p = 0;
	int q = 0;
	int r = 0;
	...
	while (1) {
		if (line[q] == ',') {
			...
			/* To get spec value as number of chanels as panel has got. */
			temp_buf = buf[tx_num][rx_num];

			for(tx_num = 0; tx_num < ROW_SIZE; tx_num++) {
				for(rx_num = 0; rx_num < COL_SIZE; rx_num++) {
					buf[tx_num][rx_num] = temp_buf;
				}
			}

			TOUCH_I("Complete to fill up spec data\n");
			break;
			/*Complete to fill up spec data for all nodes with representative value */
			r++;
			if (r % (int)COL_SIZE == 0) {
				rx_num = 0;
				tx_num++;
...
static int prd_compare_os_data(struct device *dev, u8 type)
{
	...
	case 	OPEN_NODE_TEST:
		...
		PRD_D(OPEN_TEST, "[ OPEN NODE DATA TEST ] \n");
		PRD_D(OPEN_TEST, "cnt [data] [lower] [upper] \n");
		break;
	case	SHORT_NODE_TEST:
		if (PANEL_VENDOR == TOVIS) {
			snprintf(lower_str, sizeof(lower_str),
		...
		PRD_D(SHORT_TEST, "[ SHORT NODE DATA TEST ] \n");
		PRD_D(SHORT_TEST, "cnt [data] [lower] [upper] \n");
		break;
	}
	...
	ret += snprintf(W_Buf + ret, BUF_SIZE - ret, "[%s : %d] [%s : %d]\n",
		lower_str, LowerImage[0][0], upper_str, UpperImage[0][0]);
	TOUCH_I("[%s : %d] [%s : %d]\n",
		lower_str, LowerImage[0][0], upper_str, UpperImage[0][0]);
...
/* //for test
				...
				if (short_buf_index > (ROW_SIZE-COL_ADD_SIZE)){ //Right
...
*/
				PRD_D(SHORT_TEST, "%02d [%d] [%d] [%d] \n", i + tc_total_row*j,
				short_buf[i + tc_total_row*j], LowerImage[i][j], UpperImage[i][j]);

				if ((short_buf[i + tc_total_row*j] < LowerImage[i][j]) ||
					(short_buf[i + tc_total_row*j] > UpperImage[i][j])) {
					result = 1;
					ret += snprintf(W_Buf + ret, BUF_SIZE - ret,
						"F [%d][%d] = %d\n", i, j, short_buf[i + tc_total_row*j]);
		...
		for (i = 0; i < ROW_SIZE; i++) {
			for (j = 0; j < col_size-1; j++) {
				PRD_D(OPEN_TEST, "%02d [%d] [%d] [%d] \n", i*col_size + j,
				open_buf[i*col_size + j], LowerImage[i][j], UpperImage[i][j]);
...
static int prd_open_short_test(struct device *dev)
{
	...
	prd_print_os_data(dev, W_Buf, type);
	open_result = prd_compare_os_data(dev, type);
	write_file(dev, W_Buf, TIME_INFO_SKIP);
	TOUCH_I("open_result = %d\n", open_result);
	...
	prd_os_xline_result_read(dev, type);
	prd_print_os_data(dev, W_Buf, type);
	short_result = prd_compare_os_data(dev, type);
	write_file(dev, W_Buf, TIME_INFO_SKIP);
	TOUCH_I("short_result = %d\n", short_result);
...
static void prd_read_rawdata(struct device *dev, u8 type)
{
	...
	int __m2_frame_size = ROW_SIZE * (COL_SIZE + COL_ADD_SIZE) * RAWDATA_SIZE;
	int i = 0;
	...
	lg4894_reg_read(dev, prod_m1_m2_raw_offset,
	...
	case U0_M1_RAWDATA_TEST:
		...
		raw_offset = m2_raw_offset[0];
		TOUCH_I("raw_offset[%d] = %0x\n", i, raw_offset);
		lg4894_reg_write(dev, tc_tsp_test_data_offset,
			(u8 *)&raw_offset, sizeof(u16));
		lg4894_reg_read(dev, tc_tsp_data_access_addr,
			(u8 *)M1_Rawdata_buf, __m1_frame_size);
...
static int prd_print_rawdata(struct device *dev, char *buf, int buf_size, u8 type)
{
	...
	int max = 0;
	u16 *m1_raw_buf = NULL;
	u16 (*m2_raw_buf)[tc_total_row * tc_total_col] = NULL;
	int col_size = 0;
...
static int prd_compare_rawdata(struct device *dev, u8 type)
{
	...
	//Average Jiiter Buf
	u16 m2_raw_average_buf[2][tc_total_row] = {{0}};
	int col_size = 0;
	int i, j, k;
	int ret = 0;
	int result = 0;
	...
	case U3_BLU_JITTER_TEST:
		if (PANEL_VENDOR == TOVIS) {
			snprintf(lower_str, sizeof(lower_str),
					"U3_BLU_JITTER_T_Lower");
			snprintf(upper_str, sizeof(upper_str),
					"U3_BLU_JITTER_T_Upper");
			snprintf(average_str, sizeof(average_str),
					"U3_BLU_JITTER_T_Average");
		} else { /* LGD_PANEL */
			snprintf(lower_str, sizeof(lower_str),
					"U3_BLU_JITTER_Lower");
			snprintf(upper_str, sizeof(upper_str),
	...
	memset(LowerImage, 0, sizeof(LowerImage));
	memset(UpperImage, 0, sizeof(UpperImage));
	memset(AverageImage, 0, sizeof(AverageImage));

	sic_get_limit(dev, lower_str, LowerImage);
	sic_get_limit(dev, upper_str, UpperImage);
	sic_get_limit(dev, average_str, AverageImage);

	PRD_D(RAWDATA_TEST, "[ prd_compare_rawdata TEST ] \n");
	PRD_D(RAWDATA_TEST, "cnt [data] [lower] [upper] \n");
	...
	if (type == U0_M1_RAWDATA_TEST) {
		for (i = 0; i < ROW_SIZE; i++) {
			for (j = 0; j < col_size; j++) {
		PRD_D(RAWDATA_TEST, "%02d [%d] [%d] [%d] \n",
			i*col_size + j, m1_raw_buf[i*col_size + j],LowerImage[i][j], UpperImage[i][j]);
                ...
                (m1_raw_buf[i*col_size + j] > UpperImage[i][j])) {
					...
					if ((type != U0_M1_RAWDATA_TEST) && (i <= 1 && j <= 4)) {
						if (m1_raw_bufi + ROW_SIZE*j] != 0) {
						result = 1;
						ret += snprintf(W_Buf + ret, BUF_SIZE - ret,
						"F [%d][%d] = %d\n", i, j, m1_raw_buf[i + ROW_SIZE*j]);
						}
					} else {
						result = 1;
						ret += snprintf(W_Buf + ret, BUF_SIZE - ret,
		...
		for (k =0; k < M1_M2_RAWDATA_TEST_CNT; k++) {
			for (i = 0; i < ROW_SIZE; i++) {
				for (j = 0; j < col_size - COL_ADD_SIZE; j++) {
		PRD_D(RAWDATA_TEST, "%02d [%d] [%d] [%d] \n",
			i*col_size + j, m2_raw_buf[k][i*col_size + j], LowerImage[i][j], UpperImage[i][j]);
				...
				for (i = 0; i < ROW_SIZE; i++) {
					for (j = 0; j < col_size; j++) {
						...
						//m2_raw_average_buf[j] += m2_raw_buf[k][i*col_size + j];
							...
							PRD_D(AVERAGE_TEST, "[Left]aver_sum:%d, data:%d\n",
								m2_raw_average_buf[0][i], m2_raw_buf[k][i*col_size+j]);
						} else {
							m2_raw_average_buf[1][i] += m2_raw_buf[k][i*col_size+j];

							PRD_D(AVERAGE_TEST, "[Right]aver_sum:%d, data:%d\n",
								m2_raw_average_buf[1][i], m2_raw_buf[k][i*col_size+j]);
						}
					}
					...
					PRD_D(AVERAGE_TEST, "left:%d, left_spec:%d, right:%d, rihgt_spec:%d \n",
					m2_raw_average_buf[0][i], AverageImage[i][0],
					m2_raw_average_buf[1][i], AverageImage[i][1]);
...
static void tune_display(struct device *dev, char *tc_tune_code,
	int offset, int type)
{
	char log_buf[tc_tune_code_size + 1] = {0,};
	int ret = 0;
	int i = 0;
	char temp[tc_tune_code_size + 1] = {0,};
...
static void read_tune_code(struct device *dev, u8 type)
{
	...
	lg4894_reg_read(dev, tune_result_offset,
		(u8 *)&tune_code_offset, sizeof(u32));
	offset = (tune_code_offset >> 16) & 0xFFFF;

	TOUCH_I("Tune Code Offset: %x \n", offset);
...
static int prd_rawdata_test(struct device *dev, u8 type)
{
	...
	case U3_BLU_JITTER_TEST:
		snprintf(test_type, sizeof(test_type),
				"[U3_BLU_JITTER_TEST]");
		TOUCH_I("[U3_BLU_JITTER_TEST]");
		break;
	case U3_JITTER_TEST:
		snprintf(test_type, sizeof(test_type),
				"[U3_JITTER_TEST]");
		TOUCH_I("[U3_JITTER_TEST]");
		break;
	case U0_JITTER_TEST:
		snprintf(test_type, sizeof(test_type),
				"[U0_JITTER_TEST]");
		TOUCH_I("[U0_JITTER_TEST]");
		break;
	default:
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	if(!ROW_SIZE || !COL_SIZE) {
		TOUCH_E("prd info not setting error(row:%d, col:%d)\n",
			ROW_SIZE, COL_SIZE);
		write_file(dev, "prd info not setting error\n"
			, TIME_INFO_SKIP);
		ret = snprintf(buf, PAGE_SIZE, "prd info not setting error\n");
		return ret;
	}
	...
	*/
	...
	if (rawdata_ret == 0 && jitter_ret == 0)//&& blu_jitter_ret == 0)
		...
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				...
				rawdata_ret? 0 : 1, jitter_ret? 0 : 1);//, blu_jitter_ret);
...
static int prd_show_prd_get_data_do_deltadata(struct device *dev, u8 *buf, int size, int flag)
{
	...
	int size_rd = (((ROW_SIZE+2)*(COL_SIZE+2))<<1);
	int row, col;
	int i;
	int ret = 0;
...
static int prd_show_prd_get_data_do_labeldata(struct device *dev, u8 *buf, int size, int flag)
{
	...
	int size_rd = (ROW_SIZE+2)*(COL_SIZE+2);
	int row, col;
	int i;
	int ret = 0;
...
static ssize_t prd_show_app_info(struct device *dev, char *buf)
{
	...
	buf[8] = ROW_SIZE;	//PRD_ROW_SIZE;
	buf[9] = COL_SIZE;	//PRD_COL_SIZE;
	buf[10] = COL_ADD_SIZE;	//PRD_COL_ADD;
	buf[11] = PRD_CH;
	buf[12] = PRD_M1_COL;
	buf[13] = PRD_CMD_TYPE;
	...
	TOUCH_I("R:%d C:%d C_A:%d CH:%d M1_C:%d \n	\
		...
		ROW_SIZE, COL_SIZE, COL_ADD_SIZE, PRD_CH, PRD_M1_COL,	\
