...
static int hx83102d_fw_compare(struct device *dev, const struct firmware *fw)
{
	...
	/*if (ts->force_fwup || (ic_data->vendor_fw_ver != bin_fw_version) \
		|| (ic_data->vendor_config_ver != bin_cfg_version))*/
	if (ts->force_fwup || (ic_data->vendor_fw_ver != bin_fw_version))
	{
		update = 1;
	}

	TOUCH_I("%s : bin_fw_version = %d,  bin_cfg_version = %d" \
		" ic_fw_ver = %d, ic_config_ver = %d,update = %d, force: %d\n", __func__,
...
void hx83102d_read_FW_ver(struct device *dev)
{
	...
	ic_data->lge_fw_ver_majr1 =  data[0];
	ic_data->lge_fw_ver_majr2 =  data[1];
	ic_data->lge_fw_ver_minor =  data[3];
	TOUCH_I("LGE FW MAJ1 : %X \n", ic_data->lge_fw_ver_majr1);
	TOUCH_I("LGE FW MAJ2 : %X \n", ic_data->lge_fw_ver_majr2);
	TOUCH_I("LGE FW MINR : %X \n", ic_data->lge_fw_ver_minor);
}
...
static int hx83102d_lpwg_mode(struct device *dev)
{
	...
	TOUCH_I("%s, fb : %d\n", __func__,atomic_read(&ts->state.fb));
...
static int hx83102d_suspend(struct device *dev)
{
	...
	if (d->suspended) {
		TOUCH_I("%s: Already suspended. Skipped. \n", __func__);
		return 0;
	} else {
		d->suspended = true;
		TOUCH_I("%s: enter \n", __func__);
...
//[jwm] To solve for no touch when first booting 
//	if (d->suspended == false) {
	if (0) {
		TOUCH_I("%s: It had entered resume, skip this step \n", __func__);
		return 0;
	} else {
		d->suspended = false;
		TOUCH_I("%s: enter \n", __func__);
