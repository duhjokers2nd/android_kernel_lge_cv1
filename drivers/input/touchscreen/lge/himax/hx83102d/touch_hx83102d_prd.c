										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		msleep(20);
		result = hx83102d_calculateChecksum(dev);
									...
									size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		temp_buf = kzalloc(len, GFP_KERNEL);
		ret += snprintf(temp_buf + ret, len - ret, "%d ", d->irq_enabled);
									...
									size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		temp_buf = kzalloc(len, GFP_KERNEL);
		ret += snprintf(temp_buf + ret, len - ret, "%d ", d->pdata->abs_x_min);
		ret += snprintf(temp_buf + ret, len - ret, "%d ", d->pdata->abs_x_max);
		ret += snprintf(temp_buf + ret, len - ret, "%d ", d->pdata->abs_y_min);
		ret += snprintf(temp_buf + ret, len - ret, "%d ", d->pdata->abs_y_max);
		ret += snprintf(temp_buf + ret, len - ret, "\n");
										...
										size_t len, loff_t *pos)
{
	...
	if (d->debug_log_level & BIT(3)) {
		if (d->pdata->screenWidth > 0 && d->pdata->screenHeight > 0 &&
			(d->pdata->abs_x_max - d->pdata->abs_x_min) > 0 &&
			(d->pdata->abs_y_max - d->pdata->abs_y_min) > 0) {
			d->widthFactor = (d->pdata->screenWidth << SHIFTBITS) / (d->pdata->abs_x_max - d->pdata->abs_x_min);
			d->heightFactor = (d->pdata->screenHeight << SHIFTBITS) / (d->pdata->abs_y_max - d->pdata->abs_y_min);
										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		hx83102d_register_read(dev, addr, 128, data, ADDR_LEN_4);
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x') {
		...
		if (buf[0] == 'r') {
			...
			if (count < 3) {
				...
				addr = (register_command[3] << 24) + (register_command[2] << 16) + (register_command[1] << 8) + register_command[0];
				hx83102d_register_write(dev, addr, byte_length, w_data, ADDR_LEN_4);
			} else {
				...
				if (strcmp(HX_85XX_H_SERIES_PWON, private_ts->chip_name) == 0 && cfg_flag == 0)
					cfg_flag = 2;
				addr = (register_command[3] << 24) + (register_command[2] << 16) + (register_command[1] << 8) + register_command[0];
				hx83102d_register_write(dev, addr, byte_length, &w_data[0], ADDR_LEN_4);
			}
		} else {
			return len;
		}
...
static bool hx83102d_diag_check_sum(struct himax_report_data *hx_touch_data)
{
	uint16_t check_sum_cal = 0;
	int i;

	/* Check 128th byte CRC */
	for (i = 0, check_sum_cal = 0; i < (hx_touch_data->touch_all_size - CUST_TP_SIZE); i += 2) { //hx_touch_data->touch_info_size
...
static void hx83102d_diag_parse_raw_data(struct himax_report_data *hx_touch_data, int mul_num, int self_num, uint8_t diag_cmd, int32_t *mutual_data, int32_t *self_data)
{
	int RawDataLen_word;
	int index = 0;
	int temp1, temp2, i;
	...
	if (hx_touch_data->hx_rawdata_buf[0] == FW_DATA_RAWDATA_READY_LB
		...
		RawDataLen_word =  ((MAX_I2C_TRANS_SZ - CUST_TP_SIZE) / 2) - 3; /* 1 byte Header, 1 byte index, 1 byte checksum*/
...
int hx83102d_set_diag_cmd(struct himax_ic_data *ic_data, struct himax_report_data *hx_touch_data)
{
	struct himax_ts_data *d = private_ts;
	int32_t *mutual_data;
	int32_t *self_data;
	...
	if (hx_touch_data->diag_cmd >= 1 && hx_touch_data->diag_cmd <= 7) {
		...
		if (!hx83102d_diag_check_sum(hx_touch_data)) {
		...
		hx83102d_diag_parse_raw_data(hx_touch_data, mul_num, self_num, hx_touch_data->diag_cmd, mutual_data, self_data);
...
void hx83102d_log_touch_data(struct himax_ts_data *d, int start)
{
	int loop_i = 0;
	int print_size = 0;
	uint8_t *buf;
...
void hx83102d_log_touch_event(struct himax_ts_data *d, int start)
{
	int loop_i = 0;
	if (g_target_report_data->finger_on > 0 && g_target_report_data->finger_num > 0) {
		for (loop_i = 0; loop_i < d->nFinger_support; loop_i++) {
			if (g_target_report_data->x[loop_i] >= 0 && g_target_report_data->x[loop_i] <= d->pdata->abs_x_max && g_target_report_data->y[loop_i] >= 0 && g_target_report_data->y[loop_i] <= d->pdata->abs_y_max) {
...
void hx83102d_log_touch_int_devation(int touched)
{
...
void hx83102d_log_touch_event_detail(struct himax_ts_data *d, int start)
{
	int loop_i = 0;
...
void hx83102d_ts_dbg_func(struct himax_ts_data *d, int start)
{
	switch (d->debug_log_level) {
	case 1:
		hx83102d_log_touch_data(d, start);
		break;
	case 2:
		hx83102d_log_touch_event(d, start);
		break;
	case 4:
		hx83102d_log_touch_int_devation(start);
		break;
	case 8:
		hx83102d_log_touch_event_detail(d, start);
		break;
	}
}
...
static void hx83102d_print_state_info(struct seq_file *s)
{
...
static void hx83102d_diag_arrange_print(struct seq_file *s, int i, int j, int transpose)
{
	if (transpose)
...
static void hx83102d_diag_arrange_inloop(struct seq_file *s, int in_init, int out_init, bool transpose, int j)
{
	...
	if (in_init > 0) { /* bit0 = 1 */
		for (i = in_init - 1; i >= 0; i--) {
			hx83102d_diag_arrange_print(s, i, j, transpose);
		}
...
/* print first step which is row */
static void hx83102d_diag_arrange_outloop(struct seq_file *s, int transpose, int out_init, int in_init)
{
	int j;
	int x_channel = ic_data->HX_RX_NUM;
	int y_channel = ic_data->HX_TX_NUM;
	...
	if (out_init > 0) { /* bit1 = 1 */
		...
		for (j = out_init - 1; j >= 0; j--) {
			...
			hx83102d_diag_arrange_inloop(s, in_init, out_init, transpose, j);
...
static void hx83102d_diag_arrange(struct seq_file *s)
{
	int x_channel = ic_data->HX_RX_NUM;
	int y_channel = ic_data->HX_TX_NUM;
	int bit2, bit1, bit0;
	...
	if (g_diag_arr_num < 4) {
		...
		seq_printf(s, "\n");
		hx83102d_diag_arrange_outloop(s, bit2, bit1 * y_channel, bit0 * x_channel);
		seq_printf(s, "%6c", ' ');
		...
		seq_printf(s, "\n");
		hx83102d_diag_arrange_outloop(s, bit2, bit1 * x_channel, bit0 * y_channel);
		seq_printf(s, "%6c", ' ');
...
/* DSRAM thread */
static int hx83102d_write_read_reg(struct device *dev, uint32_t tmp_addr, uint8_t *tmp_data, uint8_t hb, uint8_t lb)
{
	int cnt = 0;

	do {
		hx83102d_register_write(dev, tmp_addr, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		msleep(2);
		hx83102d_register_read(dev, tmp_addr, 4, tmp_data, ADDR_LEN_4);
		/* TOUCH_I("%s:Now tmp_data[0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
		 __func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);*/
	} while ((tmp_data[1] != hb && tmp_data[0] != lb) && cnt++ < 9);
...
static bool hx83102d_get_DSRAM_data(struct device *dev, uint8_t *info_data, bool DSRAM_Flag)
{
	int i = 0;
	...
	fw_run_flag = hx83102d_write_read_reg(dev, SRAM_ADR_RAWDATA_ADDR, tmp_data, Data_PWD0, Data_PWD1); //A5,5A
	...
	for (i = 0; i < total_read_times; i++) {
		address = (SRAM_ADR_RAWDATA_ADDR << 24) +
		(SRAM_ADR_RAWDATA_ADDR << 16) +
		(SRAM_ADR_RAWDATA_ADDR << 8) +
		SRAM_ADR_RAWDATA_ADDR + i * max_i2c_size;
		...
		if (total_size_temp >= max_i2c_size) {
			...
			hx83102d_register_read(dev, addr, max_i2c_size, &temp_info_data[i * max_i2c_size], ADDR_LEN_4);
			total_size_temp = total_size_temp - max_i2c_size;
			TOUCH_I("addr = 0x%08X\n", addr);
		} else {
			/*TOUCH_I("last total_size_temp=%d\n",total_size_temp);*/
			addr = (tmp_addr[3] << 24) + (tmp_addr[2] << 16) + (tmp_addr[1] << 8) + tmp_addr[0];
			hx83102d_register_read(dev, addr, total_size_temp % max_i2c_size, &temp_info_data[i * max_i2c_size], ADDR_LEN_4);
		}
	}
	...
	if (DSRAM_Flag == false || private_ts->diag_cmd == 0) {
		/*TOUCH_I("Return to Event Stack!\n");*/
		for (i = 0; i < ADDR_LEN_4; i++) {
			w_data[i] = (uint8_t)(SRAM_CMD_FIN >> i * 8);
		}
	} else {
		/*TOUCH_I("Continue to SRAM!\n");*/
		for (i = 0; i < ADDR_LEN_4; i++) {
			w_data[i] = (uint8_t)(SRAM_CMD_CONTI >> i * 8);
		}
	}
	hx83102d_register_write(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, w_data, ADDR_LEN_4);
...
bool hx83102d_ts_diag_func(void)
{
	struct device *dev = g_hx_dev;
	...
	if (hx83102d_get_DSRAM_data(dev, info_data, DSRAM_Flag) == false)
		return false;
...
void hx83102d_reload_disable(struct device *dev, int disable)
{
	uint8_t w_data[DATA_LEN_4];
	int i = 0;
	...
	hx83102d_register_write(dev, DRIVER_ADDR_FW_DEFINE_FLASH_RELOAD, DATA_LEN_4, w_data, ADDR_LEN_4);

	TOUCH_I("%s: setting OK!\n", __func__);
}

static int hx83102d_determin_diag_rawdata(int diag_command)
{
	return diag_command % 10;
}

static int hx83102d_determin_diag_storage(int diag_command)
{
	return diag_command / 10;
}

static void hx83102d_return_event_stack(struct device *dev)
{
	int retry = 20;
	int i = 0;
	...
	do {
		...
		hx83102d_register_write(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		hx83102d_register_read(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		retry--;
		msleep(10);
...
static void hx83102d_idle_mode(struct device *dev, int disable)
{
	int retry = 20;
	uint8_t tmp_data[DATA_LEN_4];
	...
	do {
		...
		hx83102d_register_read(dev, FW_ADDR_FW_MODE_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		...
		hx83102d_register_write(dev, FW_ADDR_FW_MODE_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		hx83102d_register_read(dev, FW_ADDR_FW_MODE_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
int hx83102d_assign_sorting_mode(struct device *dev, uint8_t *tmp_data)
{
	...
	hx83102d_register_write(dev, FW_ADDR_SORTING_MODE_EN, DATA_LEN_4, tmp_data, ADDR_LEN_4);

	return NO_ERR;
}

static int hx83102d_check_sorting_mode(struct device *dev, uint8_t *tmp_data)
{

	hx83102d_register_read(dev, FW_ADDR_SORTING_MODE_EN, DATA_LEN_4, tmp_data, ADDR_LEN_4);
	TOUCH_I("%s: tmp_data[0]=%x,tmp_data[1]=%x\n", __func__, tmp_data[0], tmp_data[1]);

	return NO_ERR;
}

static int hx83102d_switch_mode(struct device *dev, int mode)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t w_data[DATA_LEN_4];
	int i = 0;
	...
	hx83102d_idle_mode(dev, 1);
	hx83102d_reload_disable(dev, 1);
	...
	if (mode) {
		...
		hx83102d_register_write(dev, DRIVER_ADDR_FW_DEFINE_RXNUM_TXNUM_MAXPT, DATA_LEN_4, w_data, ADDR_LEN_4);
	} else {
		for (i = 0; i < ADDR_LEN_4; i++) {
			w_data[i] = (uint8_t)(FW_DATA_SET_FRAME >> i * 8);
		}
		hx83102d_register_write(dev, FW_ADDR_SET_FRAME_ADDR, DATA_LEN_4, w_data, ADDR_LEN_4);
		for (i = 0; i < ADDR_LEN_4; i++) {
	...
	while (retry != 0) {
		TOUCH_I("[%d] %s Read\n", retry, __func__);
		hx83102d_check_sorting_mode(dev, tmp_data);
		msleep(100);
		...
		hx83102d_register_read(dev, FW_ADDR_CHK_FW_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
void hx83102d_diag_register_set(struct device *dev, uint8_t diag_command, uint8_t storage_type)
{
	uint8_t tmp_data[DATA_LEN_4];
	...
	do {
		...
		hx83102d_register_read(dev, FW_ADDR_RAW_OUT_SEL, DATA_LEN_4, back_data, ADDR_LEN_4);
...
static ssize_t store_diag(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	struct device *dev = g_hx_dev;
	struct himax_ts_data *d = to_himax_data(dev);
	char messages[80] = {0};
	...
	storage_type = hx83102d_determin_diag_storage(d->diag_cmd);
	rawdata_type = hx83102d_determin_diag_rawdata(d->diag_cmd);
	...
	if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
		...
		if (DSRAM_Flag) {
			...
			hx83102d_return_event_stack(dev);
		}

		if (g_switch_mode == 2) {
			hx83102d_idle_mode(dev, 0);
			g_switch_mode = hx83102d_switch_mode(dev, 0);
		}
		...
		hx83102d_diag_register_set(dev, command[0], storage_type);
		...
		hx83102d_idle_mode(dev, 1);
		g_switch_mode = hx83102d_switch_mode(dev, 1);
								...
								size_t len, loff_t *pos)
{
	...
	hx83102d_set_SMWP_enable(dev, d->SMWP_enable);
								 ...
								 size_t len, loff_t *pos)
{
	...
	if (buf_tmp[0] == '1') {
		...
		hx83102d_ic_reset(dev, false, true);
	} else if (buf_tmp[0] == '3') {
		hx83102d_ic_reset(dev, true, false);
	} else if (buf_tmp[0] == '4') {
		hx83102d_ic_reset(dev, true, true);
	}
...
static int hx83102d_read_ic_trigger_type(struct device *dev)
{
	uint8_t tmp_data[DATA_LEN_4];
	int trigger_type = false;
	hx83102d_register_read(dev, FW_ADDR_TRIGGER_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
static int hx83102d_read_i2c_status(void)
{
								...
								size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (debug_level_cmd == 't') {
			...
			ret += snprintf(temp_buf + ret, len - ret, "LGE_FW_VER: %2X.%2X \n", ic_data->lge_fw_ver_majr1, ic_data->lge_fw_ver_majr2);
			ret += snprintf(temp_buf + ret, len - ret, "LGE_MINOR_VER = 0x%2.2X \n", ic_data->lge_fw_ver_minor);
			...
			if (hx83102d_read_ic_trigger_type(dev) == 1) /* Edgd = 1, Level = 0 */
				ret += snprintf(temp_buf + ret, len - ret, "IC Interrupt type is edge trigger.\n");
			else if (hx83102d_read_ic_trigger_type(dev) == 0)
								 ...
								 size_t len, loff_t *pos)
{
	...
	if (buf[0] == 'v') { /* firmware version */
		...
		switch (fw_type) {
		case 64:
			if (hx83102d_fts_ctpm_fw_upgrade_with_sys_fs_64k(dev, (unsigned char *)fw->data, fw->size) == 0) {
...
static int hx83102d_read_FW_status(struct device *dev, uint8_t *state_addr, uint8_t *tmp_addr)
{
	uint8_t i;
	uint8_t req_size = 0;
	...
	if (state_addr[0] == 0x01) {
		...
		hx83102d_register_read(dev, addr, req_size, tmp_addr, ADDR_LEN_4);
										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (hx83102d_read_FW_status(dev, cmd_set, tmp_data) == NO_ERR) {
...
uint8_t hx83102d_read_DD_status(struct device *dev, uint8_t *cmd_set, uint8_t *tmp_data)
{
	int cnt = 0;
	uint8_t req_size = cmd_set[0];
	cmd_set[3] = FW_DATA_DD_REQUEST;
	hx83102d_register_write(dev, FW_ADDR_DD_HANDSHAK_ADDR, DATA_LEN_4, cmd_set, ADDR_LEN_4);
	...
	for (cnt = 0; cnt < 100; cnt++) {
		hx83102d_register_read(dev, FW_ADDR_DD_HANDSHAK_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		msleep(10);
	...
	hx83102d_register_read(dev, FW_ADDR_DD_DATA_ADDR, req_size, tmp_data, ADDR_LEN_4);
	return NO_ERR;
}
										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (mutual_set_flag == 1) {
			if (hx83102d_read_DD_status(dev, cmd_set, tmp_data) == NO_ERR) {
...
static void hx83102d_flash_dump_func(struct device *dev, uint8_t local_flash_command, int Flash_Size, uint8_t *flash_buffer)
{
	...
	for (page_prog_start = 0; page_prog_start < Flash_Size; page_prog_start += 128) {
		...
		hx83102d_register_read(dev, addr, 128, buffer, ADDR_LEN_4);
...
void hx83102d_ts_flash_func(void)
{
	struct device *dev = g_hx_dev;
	struct himax_ts_data *d = to_himax_data(dev);
	uint8_t local_flash_command = 0;
	...
	if ((local_flash_command == 1 || local_flash_command == 2) || (local_flash_command == 0x0F)) {
		hx83102d_flash_dump_func(dev, local_flash_command, Flash_Size, flash_buffer);
	}
...
static void hx83102d_dbg_data_init(void)
{
	debug_data->fp_ts_dbg_func = hx83102d_ts_dbg_func;
	debug_data->fp_set_diag_cmd = hx83102d_set_diag_cmd;
	debug_data->flash_dump_going = false;
}

static void hx83102d_ts_flash_work_func(struct work_struct *work)
{
	hx83102d_ts_flash_func();
}

static void hx83102d_ts_diag_work_func(struct work_struct *work)
{
	hx83102d_ts_diag_func();
}

int hx83102d_touch_proc_init(void)
{
...
void hx83102d_touch_proc_deinit(void)
{
...
int hx83102d_debug_init(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	int err = 0;
	...
	INIT_WORK(&d->flash_work, hx83102d_ts_flash_work_func);
	setSysOperation(0);
	setFlashBuffer();
	...
	INIT_DELAYED_WORK(&d->himax_diag_delay_wrok, hx83102d_ts_diag_work_func);
...
int hx83102d_debug_remove(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);

	hx83102d_touch_proc_deinit();
...
void hx83102d_proc_deinit(void)
{
...
static int prd_switch_mode(struct device *dev, int mode)
{
	...
	hx83102d_register_write(dev, SRAM_ADR_RAWDATA_ADDR, 4, tmp_data, ADDR_LEN_4);
...
static int prd_get_rawdata(struct device *dev, uint32_t RAW[], uint32_t datalen)
{
	...
	while (retry < 300) {
		...
		hx83102d_register_read(dev, SRAM_ADR_RAWDATA_ADDR, 4, tmp_data, ADDR_LEN_4);
	...
	while (retry < 10) {
		...
		for (i = 0; i < (total_read_times); i++) {
			if (total_size_temp >= max_i2c_size) {
				addr = (tmp_addr[3] << 24) + (tmp_addr[2] << 16) + (tmp_addr[1] << 8) + tmp_addr[0];
				hx83102d_register_read(dev, addr, max_i2c_size, &tmp_rawdata[i*max_i2c_size], ADDR_LEN_4);
				total_size_temp = total_size_temp - max_i2c_size;
			} else {
				/*TOUCH_I("last total_size_temp=%d\n", total_size_temp);*/
				addr = (tmp_addr[3] << 24) + (tmp_addr[2] << 16) + (tmp_addr[1] << 8) + tmp_addr[0];
				hx83102d_register_read(dev, addr, total_size_temp % max_i2c_size, &tmp_rawdata[i*max_i2c_size], ADDR_LEN_4);
			}
...
static void prd_switch_data_type(struct device *dev, uint8_t checktype)
{
	...
	hx83102d_diag_register_set(dev, datatype, 0x00);
}
...
static void prd_set_N_frame(struct device *dev, uint16_t Nframe, uint8_t checktype)
{
	...
	hx83102d_register_write(dev, FW_ADDR_SET_FRAME_ADDR, 4, tmp_data, ADDR_LEN_4);

	/*skip frame*/
	hx83102d_register_read(dev, DRIVER_ADDR_FW_DEFINE_RXNUM_TXNUM_MAXPT, 4, tmp_data, ADDR_LEN_4);
	...
	hx83102d_register_write(dev, DRIVER_ADDR_FW_DEFINE_RXNUM_TXNUM_MAXPT, 4, tmp_data, ADDR_LEN_4);
}
...
static uint32_t prd_wait_sorting_mode(struct device *dev, uint8_t checktype)
{
	...
	do {
		...
		hx83102d_register_read(dev, FW_ADDR_CHK_FW_STATUS, 4, tmp_data, ADDR_LEN_4);
		TOUCH_I("%s: 0x900000A8, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n", __func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

		hx83102d_register_read(dev, FW_ADDR_CHK_FW_STATUS_2, 4, tmp_data, ADDR_LEN_4);
		TOUCH_I("%s: 0x900000E4, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n", __func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

		hx83102d_register_read(dev, FW_ADDR_CHK_FW_STATUS_3, 4, tmp_data, ADDR_LEN_4);
...
uint32_t prd_mp_test(struct device *dev, uint8_t checktype, uint32_t RAW[], uint32_t datalen)
{
	...
	if (prd_check_mode(dev, checktype)) {
		...
		if (checktype == HIMAX_INSPECTION_NOISE) {
			...
			TOUCH_I("N frame = %d\n", 1);
			prd_set_N_frame(dev, 1, checktype);
		} else {
			prd_set_N_frame(dev, 2, checktype);
		}

		hx83102d_sense_on(dev, 1);
...
static void prd_log_size_check(struct device *dev)
{
	...
	fname = "/data/touch_self_test.txt";
	if (fname) {
...
static void prd_write_file(struct device *dev, char *data, int write_time)
{
	int fd = 0;
	char *fname = NULL;
...
static ssize_t prd_mutual_data_read(size_t count, char *buf, int16_t RAW[])
{
	...
	for (i = 0; i < ic_data->HX_TX_NUM; i++) {
		count += snprintf(buf + count, PAGE_SIZE - count, "%c%2d%c",'[', i + 1,']');
		for (j = 0; j < ic_data->HX_RX_NUM; j++) {
			count += snprintf(buf + count, PAGE_SIZE - count, " %5d", RAW[ j + i*ic_data->HX_RX_NUM]);
		}
...
int prd_get_raw_data(struct device *dev, uint8_t diag_cmd, uint16_t mutual_num, int16_t RAW[])
{
	...
	hx83102d_register_write(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, w_data, ADDR_LEN_4);
	hx83102d_diag_register_set(dev, diag_cmd, 0);
	hx83102d_get_DSRAM_data(dev, info_data, true);
	hx83102d_diag_register_set(dev, 0, 0);
	for (i = 0; i < ADDR_LEN_4; i++) {
		w_data[i] = (uint8_t)(SRAM_CMD_FIN >> i * 8);
	}
	hx83102d_register_write(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, w_data, ADDR_LEN_4);
...
static int prd_print_data(struct device *dev, char *buf, uint32_t RAW[], u8 type)
{
	...
	case JITTER_TEST:
		snprintf(test_type, sizeof(test_type),
				"\n[JITTER_TEST]\n");
		break;
	case LPWUG_RAW_DATA_TEST:
		snprintf(test_type, sizeof(test_type),
				"\n[LPWUG_RAWDATA_TEST]\n");
		break;
	case LPWUG_JITTER_TEST:
		snprintf(test_type, sizeof(test_type),
				"\n[LPWUG_JITTER_TEST]\n");
		break;
	default:
	...
	/* Test Type Write */
	prd_write_file(dev, test_type, TIME_INFO_SKIP);
	...
	for (i = 0; i < ic_data->HX_TX_NUM; i++) {
		...
		for (j = 0; j < col_size; j++) {
			...
			mul_max = max(mul_max, (int)RAW[i*col_size+j]);
			mul_min = min(mul_min, (int)RAW[i*col_size+j]);
		}
	}
...
static int prd_compare_rawdata(struct device *dev, char *buf, u8 type, uint32_t RAW[])
{
	...
	switch (type) {
	case OPEN_TEST:
		mutual_upper = MUL_OPEN_TEST_THX_UP;
		mutual_lower = MUL_OPEN_TEST_THX_LOW;
		break;
	case MICRO_OPEN_TEST:
		mutual_upper = MUL_MICRO_OPEN_THX_UP;
		mutual_lower = MUL_MICRO_OPEN_THX_LOW;
		break;
	case SHORT_TEST:
		mutual_upper = MUL_SHORT_THX_UP;
		mutual_lower = MUL_SHORT_THX_LOW;
		break;
	case RAW_DATA_TEST:
	...
	case JITTER_TEST:
		...
		break;
	case LPWUG_RAW_DATA_TEST:
		mutual_upper = MUL_LPWUG_RAW_DATA_THX_UP;
		mutual_lower = MUL_LPWUG_RAW_DATA_THX_LOW;
		break;
	case LPWUG_JITTER_TEST:
		mutual_upper = JITTER_THX;
		mutual_lower = 0;
	...
	if (type == RAW_DATA_TEST || type == OPEN_TEST || type == MICRO_OPEN_TEST 
		|| type == SHORT_TEST|| type == LPWUG_RAW_DATA_TEST) {
		for (i = 0; i < ic_data->HX_TX_NUM; i++) {
			for (j = 0; j < col_size; j++) {
				if ((RAW[i * col_size + j] < mutual_lower) ||
						(RAW[i * col_size + j] > mutual_upper)) {
					result = 1;
					fail_num++;
					ret += snprintf(W_Buf + ret, BUF_SIZE - ret,
							"F Mutual[%d][%d] = %d\n", i, j, RAW[i * col_size + j]);
					TOUCH_I("F Type%d Mutual[%d][%d] = %d\n", type, i, j, RAW[i * col_size + j]);
				}
			}
		}
	}

	if (type == JITTER_TEST || type == LPWUG_JITTER_TEST) {
		for (i = 0; i < ic_data->HX_TX_NUM; i++) {
			for (j = 0; j < col_size; j++) {
				if (RAW[i * col_size + j] > mutual_upper) { //MutualUpperImage[i][j]
					result = 1;
					fail_num++;
					ret += snprintf(buf + ret, PAGE_SIZE - ret,
							"F Mutual[%d][%d] = %d\n", i, j, RAW[i * col_size + j]);
					TOUCH_I("F Type%d Mutual[%d][%d] = %d, MutualUpperImage[%d][%d]= %d \n", type, i, j, RAW[i * col_size + j], i, j, mutual_upper);
				}
			}
		}
	}
	if(result == 1)
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Test Fail : %d\n", fail_num);
	else
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Test PASS : No Errors\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
...
static int prd_rawdata_test(struct device *dev, char *buf, u8 type)
{
	...
	case JITTER_TEST:
		...
		break;
	case LPWUG_RAW_DATA_TEST:
		check_type = HIMAX_INSPECTION_LPWUG_RAWDATA;
		break;
	case LPWUG_JITTER_TEST:
		check_type = HIMAX_INSPECTION_LPWUG_NOISE;
		break;
	default:
		TOUCH_I("Test Type not defined\n");
		return 1;
...
//	memcpy(Rawdata_buf, mutual_data, MUTUAL_NUM * sizeof(uint16_t));
	...
	prd_write_file(dev, W_Buf, TIME_INFO_SKIP);
...
//	openshort_ret = prd_open_short_test(dev, buf);

	openshort_ret += prd_rawdata_test(dev, buf, OPEN_TEST);
	/*openshort_ret += prd_rawdata_test(dev, buf, MICRO_OPEN_TEST);*/
	openshort_ret += prd_rawdata_test(dev, buf, SHORT_TEST);
...
//jwm temp
	...
	if (rawdata_ret == 0)
		...
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Raw Data : Fail\n");

	if (openshort_ret == 0)
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Channel Status : Pass\n");
	else
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Channel Status : Fail\n");

	if (blu_jitter_ret == 0)
		ret += snprintf(buf + ret, PAGE_SIZE - ret,	"Jitter : Pass\n");
	else
		ret += snprintf(buf + ret, PAGE_SIZE - ret,	"Jitter : Fail\n");

	prd_write_file(dev, buf, TIME_INFO_SKIP);
	prd_write_file(dev, "Show_sd Test End\n", TIME_INFO_WRITE);
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	rawdata_ret = prd_rawdata_test(dev, buf, LPWUG_RAW_DATA_TEST);

	blu_jitter_ret = prd_rawdata_test(dev, buf, LPWUG_JITTER_TEST);

	ret = snprintf(buf, PAGE_SIZE,
			"\n========RESULT=======\n");
	if (rawdata_ret == 0)
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "LPWG Raw Data : Pass\n");
	else
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "LPWG Raw Data : Fail\n");

	if (blu_jitter_ret == 0)
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "LPWG Jitter : Pass\n");
	else
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "LPWG Jitter : Fail\n");
...
int hx83102d_register_sysfs(struct device *dev)
{
