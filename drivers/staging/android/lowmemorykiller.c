...
struct task_struct *calc_hswap_kill_score(int shrink_task_cnt, int *rss_size)
{
	...
	while (!list_empty(&stl_by_time)) {
		struct sorted_task *cursor_count;
		...
		list_for_each_entry(cursor_count, &stl_by_count, list) {
			if (cursor->tp->pid == cursor_count->tp->pid) {
				list_del(&cursor_count->list);
				comp_task.tp = cursor_count->tp;
				comp_task.score += cursor_count->score;
				break;
			}
		}
		...
		if (comp_task.score < victim_task.score) {
			...
			if (comp_task.tasksize > victim_task.tasksize) {
				victim_task.tp = comp_task.tp;
				victim_task.tasksize = comp_task.tasksize;
			}
		}
...
{
	long avg_latency = 0;
#if !defined(CONFIG_MACH_MSM8917_CV3_LAO_COM) && !defined(CONFIG_MACH_MSM8917_LV7_LAO_COM)
	if (!current_is_kswapd()) {
...
#else
		lowmem_print(1, "Killing '%s' (%d), adj %hd, reclaimable cnt %d, top (%ld, %d), reason %d\n"
#endif
				"   to free %ldkB on behalf of '%s' (%d) because\n" \
				"   cache %ldkB is below limit %ldkB for oom_score_adj %hd\n" \
				"   Free memory is %ldkB above reserved.\n" \
