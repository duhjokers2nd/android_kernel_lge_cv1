...
struct lge_time_to_full_data {
	...
	int cdp_comp;
	int dcp_comp;
#ifdef CONFIG_LGE_PM_TIME_TO_FULL_AVG_CURRENT
	int avg_sdp_comp;
	int avg_cdp_comp;
	int avg_dcp_comp;
	int avg_comp;
...
static void lge_time_to_full_evaluate_report(long eoc, struct lge_time_to_full_data *ttf) {
	...
	rc = ttf->usb_psy->get_property(ttf->usb_psy, POWER_SUPPLY_PROP_REAL_TYPE, &prop);
	if (rc)
...
static int lge_time_to_full_get_charging_current(struct lge_time_to_full_data *ttf)
{
	...
	switch (usb_supply_type) {
		...
		case POWER_SUPPLY_TYPE_USB:
			...
			if (charge_current == 0 && lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO)
				charge_current = DEFAULT_SDP_CURRENT;
		...
		case POWER_SUPPLY_TYPE_USB_DCP:
			...
			rc = ttf->usb_psy->get_property(ttf->usb_psy, POWER_SUPPLY_PROP_INCOMPATIBLE_CHG, &prop);
			if (rc) {
				pr_ttf(PR_ERROR, "usb_psy incompatible chg read fail rc = %d\n", rc);
				return ERROR;
			} else {
				if (prop.intval) {
					rc = ttf->usb_psy->get_property(ttf->usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &prop);
					if (rc) {
						pr_ttf(PR_ERROR, "usb_psy incompatible chg current max read fail rc = %d\n", rc);
						return ERROR;
					} else {
						charge_current = prop.intval/1000;
					}
					...
					pr_ttf(PR_ERROR, "Floated Charger Detected\n");
					break;
				}
			}
#endif
...
#elif defined(CONFIG_MACH_SDM450_CV7A_LAO_COM) || defined(CONFIG_MACH_SDM450_CV7A_LAO_COM_NONAB)
	if (lge_sku_carrier == TMUS || lge_sku_carrier == NA_ALL)
#elif defined(CONFIG_MACH_SDM450_MH4_LAO_COM) || defined(CONFIG_MACH_SDM450_MH4_LAO_COM_NONAB)
	if (lge_sku_carrier == TMUS || lge_sku_carrier == NA_ALL)	    
#else
	if (lge_sku_carrier < LAO)
#endif
		node = of_find_compatible_node(NULL, NULL, "lge,time-to-full-sku_div_1");
	else
		node = of_find_compatible_node(NULL, NULL, "lge,time-to-full-common");

	if (!node) {
		pr_err("unable to find DT\n");
		return;
	}
#endif
...
#endif
				/* for evaluate */
				if (soc != prev_soc && chg_status == POWER_SUPPLY_STATUS_CHARGING)
					lge_time_to_full_evaluate(ttf);
...
static void ttf_external_power_changed(struct power_supply *psy)
{
	...
	if (ttf->battery_profile_read_done || lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO) {
		if (chg_present != prev_chg_present) {
			if (chg_present)
