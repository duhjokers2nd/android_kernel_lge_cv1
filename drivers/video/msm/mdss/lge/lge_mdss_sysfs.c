...
extern ssize_t set_comfort_view(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
extern ssize_t get_comfort_view(struct device *dev, struct device_attribute *attr, char *buf);
#endif
		...
		struct device_attribute *attr, char *buf)
{
	...
	switch(panel_type) {
		...
		case LGD_INCELL_SW49107_HD:
			ret = snprintf(buf, PAGE_SIZE, "LGD-SW49107_HD\n");
			break;
		case LGD_INCELL_SW49107:
			ret = snprintf(buf, PAGE_SIZE, "LGD-SW49107\n");
			break;
		case TOVIS_INCELL_SW49107_HD:
			ret = snprintf(buf, PAGE_SIZE, "TOVIS-SW49107_HD\n");
			break;
		case TOVIS_INCELL_SW49107:
			ret = snprintf(buf, PAGE_SIZE, "TOVIS-SW49107\n");
			break;
		...
		case CV1_TCL_FT8006M:
			ret = snprintf(buf, PAGE_SIZE, "TCL-FT8006M\n");
			break;
		case BOE_INCELL_ILI9881H:
			ret = snprintf(buf, PAGE_SIZE, "BOE-ILI9881H\n");
			break;
		case CSOT_INCELL_FT8736:
			ret = snprintf(buf, PAGE_SIZE, "CSOT-FT8736\n");
			break;
		case BOE_INCELL_HX83102D:
			ret = snprintf(buf, PAGE_SIZE, "BOE-HX83102D\n");
			break;
		default:
...
#endif
#if IS_ENABLED(CONFIG_LGE_DISPLAY_COMFORT_MODE)
static DEVICE_ATTR(comfort_view, S_IRUGO | S_IWUSR, get_comfort_view, set_comfort_view);
#endif
#if IS_ENABLED(CONFIG_LGE_DISPLAY_DEBUG)
