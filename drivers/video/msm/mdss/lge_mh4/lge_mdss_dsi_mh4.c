...
void lge_panel_enter_deep_sleep(void)
{
	switch (lge_get_panel_type()) {
		...
		case CSOT_INCELL_FT8736:
			ft8736_panel_enter_deep_sleep();
			break;
		case BOE_INCELL_HX83102D:
			hx83102d_panel_enter_deep_sleep();
			break;
		default:
			break;
	}
...
void lge_panel_exit_deep_sleep(void)
{
	...
	switch (lge_get_panel_type()) {
		case BOE_INCELL_ILI9881H:
			...
			break;
		case CSOT_INCELL_FT8736:
			ft8736_panel_exit_deep_sleep();
			break;
		case BOE_INCELL_HX83102D:
                        hx83102d_panel_exit_deep_sleep();
                        break;
		default:
			break;
	}
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		if (!pinfo->cont_splash_enabled) {
			...
			if(lge_get_panel_type() != CSOT_INCELL_FT8736)
...
int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if (lge_mdss_dsi_panel_power_seq_all()) {
		...
		if(lge_get_panel_type()== BOE_INCELL_ILI9881H || lge_get_panel_type()== CSOT_INCELL_FT8736){
...
void mdss_dsi_ctrl_shutdown(struct platform_device *pdev)
{
	switch (lge_get_panel_type()) {
		case BOE_INCELL_ILI9881H:
			...
			break;
		case CSOT_INCELL_FT8736:
			ft8736_mdss_dsi_ctrl_shutdown(pdev);
			break;
		case BOE_INCELL_HX83102D:
			hx83102d_mdss_dsi_ctrl_shutdown(pdev);
			break;
		default:
			break;
	}
