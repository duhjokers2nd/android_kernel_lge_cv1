...
#if defined(CONFIG_LGE_AUDIO_EFFECT) || defined(CONFIG_LGE_AUDIO_NORMALIZER) || defined(CONFIG_LGE_AUDIO_MABL) || defined(CONFIG_SND_LGE_DTS)
	if (open.preprocopo_id == ASM_STREAM_POSTPROC_TOPO_ID_DEFAULT_LGE ||
		open.preprocopo_id == ASM_STREAM_POSTPROC_TOPO_ID_OFFLOAD_LGE || open.preprocopo_id == ASM_STREAM_POSTPROC_TOPO_ID_OFFLOAD_DTS_LGE )
				   ...
				   bool overwrite_topology, int topology)
{
	...
	if (open.postprocopo_id == ASM_STREAM_POSTPROC_TOPO_ID_DEFAULT_LGE ||
		open.postprocopo_id == ASM_STREAM_POSTPROC_TOPO_ID_OFFLOAD_LGE || open.postprocopo_id == ASM_STREAM_POSTPROC_TOPO_ID_OFFLOAD_DTS_LGE)
...
int q6asm_set_lgesoundeffect_geq(struct audio_client *ac, int geq_band, int geq_gain)
{
	...
	rc = wait_event_timeout(ac->cmd_wait,
			(atomic_read(&ac->cmd_state_pp) == 0), 0.5*HZ);
	if (!rc) {
		pr_err("%s: timeout, set-params paramid[0x%x]\n", __func__,
						lgesoundeffect_geq.data.param_id);
		rc = -EINVAL;
...
int q6asm_set_lge_fadeEffect(struct audio_client *ac, int nfade)
{
	struct asm_lgesoundmabl_param_lrbalancecontrol lgesoundmabl_lrbalancecontrol;
	int sz = 0;
	int rc  = 0;
    #define CAPI_V2_PARAM_LGE_MABL_FADE_EFFECT_PARAM    0x1000b015
	...
	lgesoundmabl_lrbalancecontrol.data.module_id = APPI_LGE_SOUND_MABL_MODULE_ID;
	lgesoundmabl_lrbalancecontrol.data.param_id =  CAPI_V2_PARAM_LGE_MABL_FADE_EFFECT_PARAM;
	lgesoundmabl_lrbalancecontrol.data.param_size = lgesoundmabl_lrbalancecontrol.param.data_payload_size - sizeof(lgesoundmabl_lrbalancecontrol.data);
	lgesoundmabl_lrbalancecontrol.data.reserved = 0;
	lgesoundmabl_lrbalancecontrol.LrBalanceControl = nfade;
